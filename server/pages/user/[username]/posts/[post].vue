...
<script lang="ts" setup>
    ...
    const post = user.value.posts.filter((post: any) => post.id === parseInt(route.params.post as string))[0];
    comments.value = (await $fetch<any>(`${config.public.API}/user/${username}/post/${post.id}/comments`)).data;
